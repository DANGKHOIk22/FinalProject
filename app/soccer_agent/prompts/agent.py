    ...
    aggregator_prompt_template = ChatPromptTemplate.from_messages([
        SystemMessage(
            content=(
                ...
                "## STEP 0 — RECONCILE BEFORE ANSWERING (MANDATORY, do this FIRST)\n"
                "Before writing anything, cross-check ALL worker findings against each other and against "
                "the time context. Do NOT synthesize until you have resolved conflicts.\n"
                "  (a) TIME conflicts (HIGHEST PRIORITY): Read the Time context (UTC+7) as 'now'. For every "
                "dated fact, decide if the event is PAST (date < now) or FUTURE (date > now). If the user "
                "asks about an upcoming/next event, DISCARD facts describing already-played events, and vice "
                "versa. If two findings disagree on the date of the same event, trust the one consistent with "
                "'now' and the user's intent; if unresolvable, say the date is uncertain.\n"
                "  (b) EVENT conflicts: If findings disagree on a score, scorer, result, lineup, or who did "
                "what, prefer the finding that is (1) most consistent with the confirmed date, (2) most "
                "specific and internally consistent. If still contradictory, state that reports differ rather "
                "than picking silently.\n"
                "  (c) DOCUMENT conflicts: When multiple sources overlap, merge the agreeing facts; for the "
                "points where they diverge, keep only what survives (a) and (b).\n"
                "  Only AFTER this reconciliation do you write the answer, using the surviving facts.\n\n"
                "## CONTENT RULES\n"
