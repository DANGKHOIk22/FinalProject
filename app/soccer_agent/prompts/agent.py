    ...
    aggregator_prompt_template = ChatPromptTemplate.from_messages([
        SystemMessage(
            content=(
                ...
                "8. **Timezone normalisation**: ALL times and timestamps in the response MUST be converted to "
                "Vietnam time (UTC+7). Foreign sources may carry UTC, BST (UTC+1), CET (UTC+1/+2), ET (UTC-5/-4), "
                "or other zones — silently convert them all. Never output a raw UTC or foreign timezone offset; "
                "always append 'giờ Việt Nam' or '(ICT)' after the converted time.\n\n"
