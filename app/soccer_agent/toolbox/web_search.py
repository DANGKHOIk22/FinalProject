...
_SYNTHESIS_SYSTEM = (
    ...
    "Follow this MANDATORY 4-step process before writing anything.\n\n"
    "## STEP 1 — Establish NOW (in UTC)\n"
    "The user message begins with: 'Current date/time (ICT/UTC+7): <timestamp>'\n"
    "That timestamp is in ICT (UTC+7). Convert it to UTC for all comparisons below:\n"
    "  NOW_utc = ICT_timestamp − 7 hours   (carry the date back if the hour goes below 00:00)\n"
    "All PAST/FUTURE comparisons use NOW_utc. If no timestamp is given, report facts "
    "without filtering and note the uncertainty.\n\n"
    ...
    "## STEP 3 — Normalize every event time to UTC, then classify PAST/FUTURE\n"
    "Article times come in many timezones and CANNOT be compared until normalized to UTC.\n"
    "For each event with a time, first convert its local time to UTC using this table:\n"
    ...
    "  (If the hour crosses a day boundary after conversion, adjust the date accordingly.)\n"
    "Then classify against NOW_utc:\n"
    "  event_utc < NOW_utc → PAST (already occurred)\n"
    "  event_utc > NOW_utc → FUTURE (not yet played)\n"
    "If an event date has no time-of-day, compare by calendar date alone.\n\n"
    "## STEP 4 — Filter, then synthesize\n"
    ...
    "  e.g. 'No upcoming matches found after the current date in the available sources.'\n\n"
    "## OUTPUT\n"
    ...
    "State every time in UTC and label it 'UTC' (e.g. '19:00 UTC on 17 June'). Do NOT convert "
    "to local time yourself — a downstream step handles the final timezone. Never invent a time "
    "that was not in the sources.\n"
